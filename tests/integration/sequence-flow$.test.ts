...
describe('sequence-flow$', () => {
  it('should run workloads one after the other', async () => {
    ...
    // At the beginning only A has been started
    ...
    // After 1s, sequence is now complete !
    await vi.advanceTimersByTimeAsync(1000);

    expect(sequence.state()).toBe(WorkloadState.Succeeded);
    expect(wklA.state()).toBe(WorkloadState.Succeeded);
    expect(wklB.state()).toBe(WorkloadState.Succeeded);
