...
describe('scheduler$', () => {
  it('should register and immediately start the given workload', async () => {
    const workload = workload$({ id: 'test-1', label: 'test', onStart: vi.fn() });
  ...
  it('should register both workloads but only start the first', async () => {
    const w1 = workload$({ id: 'test-1', label: 'test', weight: 1, onStart: vi.fn() });
    vi.spyOn(w1, 'start');

    const w2 = workload$({ id: 'test-2', label: 'test', weight: 1, onStart: vi.fn() });
    vi.spyOn(w2, 'start');
